...
type Sort = "elo" | "rating" | "kd" | "matches" | "points"

const SORTS: { id: Sort; label: string }[] = [
  { id: "elo", label: "По FACEIT ELO" },
  { id: "points", label: "По очкам сезона" },
...
export function PlayerTable({
  ...
  const list = React.useMemo(() => {
    ...
    const filtered = rows.filter((player) =>
      needle ? `${player.nickname} ${player.faceit ?? ""}`.toLowerCase().includes(needle) : true,
    )
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="glass overflow-hidden rounded-xl">
        <div className="hidden grid-cols-[44px_minmax(0,1.6fr)_120px_100px_90px_90px_80px_80px] items-center gap-4 border-b border-white/[0.07] px-5 py-3 text-[10.5px] font-medium uppercase tracking-[0.14em] text-white/30 lg:grid">
          <span>#</span>
          <span>Игрок</span>
          <span>FACEIT</span>
          ...
          {list.map((player, index) => (
            <li key={player.id}>
              <Link
                ...
                className="grid grid-cols-[36px_minmax(0,1fr)_auto] items-center gap-4 px-4 py-3 transition-colors hover:bg-white/[0.03] sm:px-5 lg:grid-cols-[44px_minmax(0,1.6fr)_120px_100px_90px_90px_80px_80px]"
              >
                <span className="mono text-[13px] text-white/30">{index + 1}</span>

                <div className="flex min-w-0 items-center gap-3">
                  <PlayerAvatar nickname={player.nickname} avatar={player.avatar} size="sm" />
