...
import type { BracketRoundDto, MatchDto, MatchSideDto } from "@/lib/types"

interface BracketContextValue {
  focus: string | null
  setFocus: (tag: string | null) => void
  openMatch: (match: MatchDto) => void
}
...
export function MatchCard({ match, compact }: { match: MatchDto; compact?: boolean }) {
  const ctx = React.useContext(BracketContext)
  const live = match.state === "live"
  const done = match.state === "done"
  const review = match.state === "review"
  const onPath = Boolean(
    ctx?.focus && (match.a?.tag === ctx.focus || match.b?.tag === ctx.focus),
  )

  const footer = match.maps.length
    ? match.maps.map((map) => `${map.map} ${map.scoreA}:${map.scoreB}`).join(" · ")
    : match.note
        ...
        {live ? (
          ...
          <span
            className={cn(
              ...
              review ? "text-prize" : "text-white/25",
            )}
          >
            {done ? "Завершен" : review ? "Апелляция" : "Ожидает"}
          </span>
        )}
      </header>
 ...
 *
 * Сетка всегда верхняя (single elimination), поэтому число матчей в
 * каждом следующем раунде ровно вдвое меньше — соединитель всегда
 * «сливающий», без прямых переходов нижней сетки.
 */
  ...
  if (!rounds.length) {
    return (
      <div className="rounded-xl border border-dashed border-white/10 py-16 text-center text-[13px] text-white/30">
        Сетка появится после жеребьевки составов.
      </div>
    )
  }
