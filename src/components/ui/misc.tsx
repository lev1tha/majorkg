...
/** Тонкая спарклайн-диаграмма на CSS — без сторонних chart-библиотек. */
    ...
    <div className={cn("flex h-8 items-end gap-[3px]", className)} aria-hidden>
      {series.map((point, index) => (
        <span
          ...
          className={cn(
            "w-full rounded-[2px]",
            tone === "accent" ? "bg-accent/35" : "bg-white/20",
            index === series.length - 1 && (tone === "accent" ? "bg-accent" : "bg-white/60"),
          )}
